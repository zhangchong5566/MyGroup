...
(function($) {
	...
	DmUploader.prototype.checkBrowser = function() {
		...
		if (!this.checkEvent('drop', this.element)
				|| !this.checkEvent('dragstart', this.element)) {
			this.settings.onFallbackMode.call(this.element,
	...
	DmUploader.prototype.processQueue = function() {
		...
		$.ajax({
			...
			xhr : function() {
				...
				if (xhrobj.upload) {
					xhrobj.upload.addEventListener('progress', function(event) {
						...
						widget.settings.onUploadProgress.call(widget.element,
								widget.queuePos, percent);
					}, false);
				}
			...
			success : function(data, message, xhr) {
				widget.settings.onUploadSuccess.call(widget.element,
						widget.queuePos, data);
			},
			error : function(xhr, status, errMsg) {
				widget.settings.onUploadError.call(widget.element,
						widget.queuePos, errMsg);
			},
...
//			}
	...
	// -- Disable Document D&D events to prevent opening the file on browser
	// when we drop them
