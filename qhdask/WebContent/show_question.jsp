...
.ask-content {
	...
	font: 14px/24px arial, "\5b8b\4f53", sans-serif, tahoma,
		"Microsoft YaHei";
}
		...
		<div class="sign-in" style="margin: 0px;">
			<div>
				<ul id="breadcrumbs-one">
					...
					<li><a href="${cxp}/questions.do">问题搜索</a></li>
					<li><a href="" class="current"><c:out value="${qbean.title}"></c:out></a></li>
				</ul>
			</div>
			<div class="user-form" style="max-width: 100%; text-align: left;">
				<fieldset class="user-form__fieldset">
					...
					<div>
						${qbean.member.loginName} | 浏览${qbean.viewCount}次 |
						<fmt:formatDate value="${qbean.createDate}"
							pattern="yyyy-MM-dd HH:mm" />
						<c:if
							test="${qbean.status==1 && qbean.member.id==member_session.id}">
				...
				<c:choose>
					<c:when
						test="${qbean.status==1 && member_session.role==2 && member_session.status==2 && member_session.id!=qbean.member.id}">
						<form id="aform" class="sign-in__form user-form__form"
							...
							<fieldset class="user-form__fieldset" style="text-align: left;">
								...
								<div
									style="margin-bottom: 5px; font-size: 13px; color: #1e1e1e;">我的回答</div>
								<script type="text/plain" id="uEditor"
									style="width:100% !important;height:200px;"></script>
							</fieldset>
							<fieldset class="user-form__fieldset">
								<label class="user-form__label" for="session_login">验证码</label>
								<input type="text" name="randomCode"
									class="sign-in__field user-form__field"
									style="width: 100px; float: left;" /> <label
									style="float: left;"><img id="radomCode"
				...
				<hr
					style="height: 3px; border: none; border-top: 3px groove skyblue;" />
				<fieldset class="user-form__fieldset">
				...
				<c:forEach items="${answers}" var="ans" varStatus="stat">
					<fieldset class="user-form__fieldset">
						...
						<div style="padding: 10px 0px;">
							<label style="float: left;">${ans.member.loginName}<c:if
									test="${ans.status==2}">
									<span
										style="background: #66cc52; color: #fff; margin-left: 20px; padding: 5px;">已采纳</span>
								</c:if></label> <label style="float: right;"><fmt:formatDate
									value="${ans.createDate}" pattern="yyyy-MM-dd HH:mm" /></label>
						</div>
						<br />
						...
						<c:if
							test="${qbean.status==1 && qbean.member.id==member_session.id}">
							<br />
							<a href="javascript:caina(${ans.id})"><span
								style="background: #66cc52; color: #fff; padding: 5px;">采纳这个回答</span></a>
						</c:if>
					</fieldset>
				</c:forEach>
			</div>
